...
def make_dictionary(rs_fmri, n_components, cache, mask, n_jobs=1):
    """Train a dictionary on the data and return the components

    Parameters
    ----------
    rs_fmri: list of strings
             input fMRI files
    n_components: int,
                  number of components required
    cache: string or joblib.Mem object,
           cache to use
    mask: nimg,
          brain mask to user
    n_jobs: int, optional,
    ...
    Returns
    -------
    components: np.array of shape (n_components, n_voxls)
                the dictionary components learned
    """
...
def fit_regressions_imgs(individual_components_imgs, data, parcellations,
                         dummy_masker, clf, i):
    individual_components = dummy_masker.transform(
        individual_components_imgs[i])
    Y = data[i]
    return fit_regressions(individual_components, Y, parcellations,
                           dummy_masker, clf)
